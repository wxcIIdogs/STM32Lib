...
#include "usartOpt.h"
static USART_TypeDef* (sg_UsartBuff[]) = {USART1,USART2,USART3,UART4,UART5,USART6,UART7,UART8}; 


RpyStatus initUsartOpt(enumUsart defUSART,u32 baud)
{
	if(baud < 100 || baud > 460800 || defUSART > UST8)
	{
		return False;
	}
	//init usart 
	USART_Init(sg_UsartBuff[defUSART],baud,0);
	return True;
}
